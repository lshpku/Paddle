...
bool CanEntailLoopCondition(For for_stmt) {
  // 1. The loop extent must be >16 or be dynamic, otherwise the loop will be
  //    completely unrolled and we have nothing to do.
  ir::Expr extent = for_stmt->extent();
  if (extent.is_constant() && extent.as_int64() <= 16) return false;

  // 2. The loop body must contains exactly one IfThenElse, the condition of
  //    which entails the loop's extent.
...
struct CommonFactorExtractor : public ir::IRMutator<> {
  ...
  int64_t GetResult() { return common_factor_; }
...
void EntailLoopCondition(For for_stmt) {
  ...
  // Try extract common factor of loop_var.
  // Note: type of common_factor should be the same as loop_var, i.e., don't
  // use int64 in an int32 expression.
  ...
  // If loop_var has a non-unit common factor, replace all occurrences of
  // loop_var with strided_loop_var = loop_var * common_factor.
  if (common_factor > 1) {
  ...
  // Declare the entailment relation on the loop condition.
  ir::Expr entail_expr =
      ir::Call::Make(Void(),
...
LogicalResult EntailLoopConditionPass::Run(ir::stmt::BlockRef block) {
  if (std::getenv("no_entail")) return LogicalResult::success();
